...
func _ready() -> void:
	...
	# A Hitbox do corpo deve estar SEMPRE pronta para receber dano [cite: 21]
...
func take_damage(amount: float = 20.0):
	...
	# Ele se move na direção oposta à que o player está (que é a direção da bola)
	var knockback_force = 100.0
	# Se a bola veio da esquerda, ele vai para a direita
...
# --- SINAIS ---
func die_instantly():
	take_damage(health) # Tira toda a vida de uma vez
